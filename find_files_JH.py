...
class Import(object):
    ...
    def divide_work(self):
        ...
        directory = '/Users/jordanhoffmann/Dropbox/10_Sports/Test10'
    ...
    def reduce_work(self,x,sports):
        ...
        if self.rank == 0:
            ...
            np.save('x_10_test_10.npy',x)
            np.save('y_10_test_10.npy',sports)          
  #          np.save('x_train_19_bw100.npy',x)
  #          np.save('y_train_19_bw100.npy',sports)

def extract_image(adress,frames_per_video = 5,frame_n = 50,dim =  100):
    
    cap = cv2.VideoCapture(adress)
    ALL = np.zeros((frames_per_video,dim**2))
    counter_1 = 0
    counter_2 = 0
