...
class ConvNet(Functions):
    ...
    def model(self,batch_size,num_kernels,kernel_sizes,x,y,sidestream_size):
            
        # convolutional layers
        conv  = Convolution(batch_size, num_kernels, kernel_sizes, 0, x[:,:100*100], y)
        #conv2 = Convolution(batch_size, num_kernels, kernel_sizes, 1, x, y)
        #conv3 = Convolution(batch_size, num_kernels, kernel_sizes, 2, x, y)
        
        sidestream_input = x[:,100*100:]
        sidestream = HiddenLayer(rng,
                                  input      = sidestream_input,
                                  n_in       = sidestream_size,
                                  n_out      = sidestream_size,
                                  activation = self.rectify)
                    

        layer2_input = T.concatenate((conv.layer1.output.flatten(2),sidestream.output),axis=1)
        ...
        layer2 = HiddenLayer(rng,
                                  ...
                                  n_in       = num_kernels[1] * conv.edge1 * conv.edge1 + sidestream_size,
    ...
    def run(self,
        ...
        sidestream_size = train_set_x.get_value(borrow=True).shape[1]-100**2
        ...
        self.model(batch_size, num_kernels, kernel_sizes, x, y, sidestream_size)
        ...
        print np.savetxt('y_vec.txt',test_set_y.get_value(borrow=True),delimiter=',')
        ...
        try:
            ...
            for epoch in range(epochs):
                ...
                # f.close()
                with open("workfile_cpu21", "a") as myfile:
                ...
                if epoch%1== 0:
                    ...
                    with file('workfile_cpu23', 'w') as outfile:
                    ...
                    with open("workfile_cpu22", "a") as myfile:
