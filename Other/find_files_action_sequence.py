...
class Import(object):
    ...
    def divide_work(self):
        ...
        directory = '/Volumes/Thomasina/Small_Train'
    ...
    def reduce_work(self,x,sports):
        ...
        if self.rank == 0:
            ...
            np.save('x_small_train2.npy',x)
            np.save('y_small_train2.npy',sports)
    ...
    def extract_image(self,adress,frame_n = 10):
        ...
        while(True):
            ...
            if (counter_1 >= start_n) and (counter_1 < start_n+self.frames_per_video):
                #print counter_1
                #exit()
                ...
                #MAT[counter_2] = gray
                ALL[0,loc*self.dim**2:(loc+1)*self.dim**2] += r.reshape((1,self.dim**2))
                ALL[1,loc*self.dim**2:(loc+1)*self.dim**2] += g.reshape((1,self.dim**2))
                ALL[2,loc*self.dim**2:(loc+1)*self.dim**2] += b.reshape((1,self.dim**2))
                counter_2 +=1
                loc +=1
            if counter_1 ==start_n+self.frames_per_video:
                break
