...
class ConvNet(Functions):
    ...
    def model(self,batch_size,num_kernels,kernel_sizes,x,y,parallel_layers):
            
        stacklist = []
        convparams = []
        # convolutional layers
        for n in xrange(parallel_layers):
            layer_name = 'conv'+str(n)

            exec(layer_name + '  = Convolution(batch_size, num_kernels, kernel_sizes, 0, x, y)')
            exec('stacklist.append(' +layer_name + '.layer1.output.flatten(2))')
            exec('convparams = convparams + ' +layer_name + '.layer1.params + '+layer_name + '.layer0.params')
            exec('self.'+layer_name + ' = '+layer_name)

        if parallel_layers == 1:
            layer2_input = stacklist[0]
        else:
            layer2_input = self.stack(stacklist)

        layer2 = HiddenLayer(rng,
                                  input      = self.dropout(layer2_input),
                                  n_in       = parallel_layers * num_kernels[1] * conv0.edge1 * conv0.edge1,
                                  n_out      = num_kernels[1] * conv0.edge1 * conv0.edge1,
        ...
        layer3 = LogisticRegression(input = self.dropout(layer2.output),
                                         n_in  = num_kernels[1] * conv0.edge1 * conv0.edge1,
                                         n_out = self.n_sports)
    ...
    def run(self,
        ...
        self.model(batch_size, num_kernels, kernel_sizes, x, y,parallel_layers)
        ...
        try:
            ...
            for epoch in range(epochs):
                ...
                # f.close()
                with open("workfile", "a") as myfile:
                ...
                if epoch%10== 0:
                    ...
                    with open("workfile2", "a") as myfile:
